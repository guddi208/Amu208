...
public class ELOQUA extends ConfigFileReader {

	private static final Logger logger = Logger.getLogger(ELOQUA.class.getName());
	String session_id;

	@Test(priority = 0, enabled = true)
	public void eloquaConnect() throws IOException {
		logger.info("************ ELOQUA CONNECTOR SESSION START  ***************************");
		try {
			session_id = BaseClassMethods.getSessionKEYAPI();
			logger.info("-----------ELOQUA CONNECTOR SESSION ID IS --" + session_id);
			System.out.println("The Session Id is :" + session_id);
			

			String connector_key_value = eloquaconn.getProperty("connector_key");
			logger.info("------------ELOQUA CONNECTOR KEY IS --" + connector_key_value);
	...
	////////Read Json file into JSONObject
			...
			try {
				jObj = (JSONObject)parser.parse(new FileReader(new File(System.getProperty("user.dir") + "\\ELOQUA-Connector\\com\\gcc\\connector\\Json\\ELOQUA.json")));
...
/////////////////////////////job id ///////////////////////////////////////
				...
				File fis1 = new File(System.getProperty("user.dir")
						+ "\\ELOQUA-Connector\\com\\gcc\\connector\\Config\\jobid.Properties");
				...
				try {
					properties1.store(fileOut1, "# LISTING ALL ELOQUA-CONNECTOR JOB  IDS");
				...
				prop.store(fileOut, "# LISTING ELOQUA-CONNECTOR SUBMISSION IDS");
				fileOut.close();
			...
			System.out.println("ERROR MSG IS :" + ex);
		}
	}

	@Test(priority = 1, enabled = true, dependsOnMethods = "eloquaConnect")
	public void submissionELOQUAstatus() throws IOException {

		try {
			String connector_key_value = eloquaconn.getProperty("connector_key");
			System.out.println("---ELOQUA connector-key ---" + connector_key_value);

			String Submission_id = eloquasub.getProperty("Submission_Id");
				...
				String message = js1.getString("message");
				 ...
				 if (!status.equals("Delivered")) {
					...
					continue;
					  ...
					  isDelivered = true;
					  respCode = 200;
					  logger.info("----------------ELOQUA SUBMISSION IS SUCCESSFULLY COMPLETED---------------------------");
						System.out.println("-----------------------ELOQUA CONNECTOR SUBMISSION IS SUCCESSFULLY COMPLETED ---------------------------");
				  }
			...
			ex.printStackTrace();
			logger.error("**************PROBLEM WITH ELOQUA CONNECTOR submissionELOQUAstatus********************");
		}
	}

///////////////////////////
	@Test(priority = 2, enabled = true, dependsOnMethods = "submissionELOQUAstatus")
	public void sessionELOQUAterminatesTC() {
		...
		try {
			...
			System.out.println(
					"!!!!!!!!!!! ELOQUA CONNECTOR SESSION TERMINATE SUCESSFULLY !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
			logger.info(
					"-----------------------ELOQUA CONNECTOR SESSION TERMINATE SUCESSFULLY------------------------");

		} catch (Exception ex) {
			logger.error(
					"**************************PROBLEM WITH ELOQUA CONNECTOR ******************************************");
			logger.error(ex);
