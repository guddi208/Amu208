...
public class GITHUB extends ConfigFileReader {
	...
	public void githubConnect() throws IOException {
		...
		try {
			...
			//report//to-do
			test=extent.createTest("GITHUB CONNECTOR");
			...
			//reports//todo
			test.info("GITHUB CONNECTOR KEY GENERATED");
			...
			//reports//todo
			test.info("Submission created and added for preprocess");
...
/////////////////////////////job id ///////////////////////////////////////
			...
			//reports //todo
			test.pass("Github submission is done");
	...
	public void submissiongithubstatus() throws IOException {
		...
		try {
			...
			while (!isDelivered) {
				...
				assertEquals(200, BaseClass.ConstantsKeywords.resp.getStatusCode());
				assertEquals("Submission Task List", message);
			...
			while (!isDelivered) {
				...
				String message = js1.getString("message");
				 ...
				 assertEquals(201, BaseClass.ConstantsKeywords.resp.getStatusCode());
				 ...
				 if (!status.equals("Delivered")) {
					...
					if (counter == maxretry) {
						System.out.println("-----------------------------!!!-FAILURE-!!!---------------------------");
						//reports //todo
						test.fail("Github submission is done");
						break;
				 	}
					continue;
				  } else {
					  isDelivered = true;
					  respCode = 200;
					  logger.info("----------------GITHUB SUBMISSION IS SUCCESSFULLY COMPLETED---------------------------");
					  System.out.println("-----------------------GITHUB CONNECTOR SUBMISSION IS SUCCESSFULLY COMPLETED ---------------------------");
				  }
			...
			//reports //todo
			test.pass("Status checking  is  Completed");
		}
		catch (Exception ex) {
			
			ex.printStackTrace();
			logger.error("**************************PROBLEM WITH GITHUB CONNECTOR submissiongithubstatus******************************************");
		}
	}
...
///////////////////////////
	...
	public void sessionGITHUBterminatesTC() {
		...
		try {
			...
			//report//todo
			test.info("Session termination  is Started"); 
			...
			//report//todo
			test.pass("GITHUB CONNECTOR SESSION TERMINATE SUCESSFULLY"); 
			...
			logger.error("**************************PROBLEM WITH GITHUB CONNECTOR ******************************************");
			//reports //todo
			test.fail("Github Session termination  is Fail");
			logger.error(ex);
		}
	}
}
